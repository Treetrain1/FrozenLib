...
package org.quiltmc.qsl.frozenblock.common.worldgen.surface_rule.impl;
...
public final class VanillaSurfaceRuleTracker<T extends SurfaceRuleContextImpl> {
    ...
    private VanillaSurfaceRuleTracker(Consumer<T> eventInvoker) {
        ...
        FrozenLifecycleEvents.END_DATA_PACK_RELOAD.register(/*SURFACE_RULES_APPLY_PHASE, */(server, resourceManager, success) -> {
            if (success && server == null) {
                this.init(resourceManager);
            }
            FrozenMain.log("AMONGUS", true);
        });
    }
    ...
    public void init(ResourceManager resourceManager) {
        FrozenMain.log("surface rules initializing", true);
    ...
    private void apply(T context, ResourceManager resourceManager) {
        ...
        context.cleanup();
        FrozenMain.log("surface rules applied", true);
    }
}
